...
修饰模式声明出来的模式会失去被匹配实体的一些性质，不过可以在 \lstinline![pattern]! 的最前面加上一个总体声明：
...
此外，回忆函数之间可以进行运算，\lstinline!f @ g! 相当于 \lstinline!func (...args) -> f(...args) @ g(...args)!。因此，我们可以用 \lstinline!and! 和 \lstinline!or! 来连接多个约束。
